#!/usr/bin/env python
"""Train a 3-way task-prior top-1 routed LoRA-MoE on LBox."""
...
def build_rows(source_jsonl: Path, *, seed: int, eval_ratio: float):
    task_to_index = {task: i for i, task in enumerate(TASKS)}
    rows = []
    for raw in read_jsonl(source_jsonl):
        task = task_name(raw)
        if task not in task_to_index:
            raise ValueError(f"Unsupported LBox task label: {task!r}")
        rows.append({
            "id": str(raw["id"]),
            ...
            "route_targets": [task_to_index[task]],
            "task_type": raw.get("task_type"),
            "casetype": raw.get("casetype"),
        })
    rows.sort(key=lambda row: hashlib.sha256(f"{seed}:task-prior:{row['id']}".encode()).hexdigest())
    eval_size = max(1, int(len(rows) * eval_ratio))
    eval_rows, train_rows = rows[:eval_size], rows[eval_size:]
    LOGGER.info("LBox task-prior rows: train=%d eval=%d tasks=%s", len(train_rows), len(eval_rows), TASKS)
...
def parse_args():
    ...
    p.add_argument("--run-name", default="lbox_task_prior_3x1_lora_moe")
...
def main():
    args = parse_args()
    if args.top_k != 1:
        raise ValueError("This task-prior run is intentionally top-1")
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    set_seed(args.seed)
    train_rows, eval_rows = build_rows(Path(args.source_jsonl), seed=args.seed, eval_ratio=args.eval_ratio)
    config = RoutedLoraConfig("meta-llama/Llama-3.1-8B-Instruct", TASKS, rank=16, alpha=32, dropout=0.05, top_k=1)
    ...
    # source_jsonl is <repo>/export/lbox/lbox_train.jsonl.
    root = Path(args.source_jsonl).parents[2]
    adapter_dirs = [root / f"checkpoints/sft_llama31_8b_lbox_{task}_ffn_only_5ep" for task in TASKS]
    missing = [str(path) for path in adapter_dirs if not (path / "adapter_model.safetensors").is_file()]
    if missing:
        raise FileNotFoundError(f"Missing task-prior adapters: {missing}")
    ...
    trainer = RoutedLoraTrainer(
        model=model, args=training_args, train_dataset=RowDataset(train_rows), eval_dataset=RowDataset(eval_rows),
    ...
    if args.hub_model_id:
        trainer.push_to_hub(commit_message="End of training: best 3-way top-1 routed LBox task-prior LoRA-MoE")
    LOGGER.info("Complete; best LBox task-prior routed LoRA-MoE saved and pushed: %s", args.hub_model_id)
