#!/bin/bash
# """Handoff note: SLURM job for evaluating only critic-specific Llama ACC algorithm adapters.
# Use this when the all-critic adapter should be skipped and the comparison should focus on the five
# critic-specialized checkpoints plus an optional vanilla baseline."""
#SBATCH --job-name=eval_acc_by_llama
...
if [ "${INCLUDE_VANILLA:-false}" = "true" ]; then
    LORA_PATHS+=("")
fi
...
if [ "${#LORA_PATHS[@]}" -eq 0 ]; then
    echo "ERROR: No by-critic LoRA checkpoints found."
    echo "Expected patterns:"
    printf '  %s\n' "${PATTERNS[@]}"
    echo "You can also pass EXTRA_LORA_PATHS=\"path1 path2 ...\"."
    exit 3
fi
...
for LORA_PATH in "${LORA_PATHS[@]}"; do
    if [ -z "$LORA_PATH" ]; then
        ...
        RUN_NAME="eval_acc_algorithm_llama_$(basename "$LORA_PATH")"
        LORA_FLAG=(
...
echo "=== All requested Llama by-critic evaluations complete ==="
