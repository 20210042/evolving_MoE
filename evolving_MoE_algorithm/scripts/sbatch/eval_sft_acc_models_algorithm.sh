#!/bin/bash
# """Handoff note: SLURM job for evaluating Gemma ACC algorithm checkpoints.
# It evaluates the vanilla Gemma model plus the newest matching all-critic and critic-specific LoRA
# checkpoints, using vLLM generation and stdin/stdout ACC scoring."""
...
for LORA_PATH in "${LORA_PATHS[@]}"; do
    if [ -z "$LORA_PATH" ]; then
        echo "vanilla: google/gemma-4-31B-it"
    else
        echo "LoRA: $LORA_PATH"
    fi
done
