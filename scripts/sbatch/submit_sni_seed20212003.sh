#!/bin/bash
# SNI 진화 제출 — seed20212003 (v6 = soft_linear WAR + 이진 3경로 차단).
#   설계 근거는 configs/sni_train_seed20212003.yaml 주석 참고.
#   1에폭 = 69,588문제 / batch 50 = 1,392스텝.
#   자원은 vllm tp_size=2라 GPU 2장 필요. 시간 48h. (20212001/2002 실측 각각 3:59:37 / 4:01:22)
#   ⚠️ TRAIN_SIZE/BATCH_SIZE 환경변수는 로그 출력용이고 실제 값은 config YAML에서 읽는다.
# Usage: bash scripts/sbatch/submit_sni_seed20212003.sh
#        RESUME=true bash scripts/sbatch/submit_sni_seed20212003.sh   # 중단 후 이어 돌리기
set -euo pipefail
REPO="${REPO:-/home/jaehoonjeong/data/MetaAgentEvolution_Release}"

SEED=20212003
DATASET="sni"
EVOL_CFG="configs/sni_train_seed20212003.yaml"
TRAIN_SIZE=69588
MAX_EPOCHS=1
BATCH_SIZE=50
