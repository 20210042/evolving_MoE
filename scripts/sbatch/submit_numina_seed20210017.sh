#!/bin/bash
# seed20210017: failure-mode scout — seed16과 동일하되 scout가 하드에러당 랜덤 오답 1개를 보고
#   '실패유형'으로 전문가를 정의. batch 100→25. config = numina_train_seed17.yaml.
# Usage: bash submit_numina_seed20210017.sh [--resume]
set -euo pipefail
...
SEED=20210017
DATASET="numina_cot"
EVOL_CFG="configs/numina_train_seed17.yaml"
TRAIN_SIZE=62185
MAX_EPOCHS=1
BATCH_SIZE=25
...
submit_one() {  # $1=resume_flag  $2=dependency_jobid(optional)
    ...
    SEED=${SEED} DATASET=${DATASET} TRAIN_SIZE=${TRAIN_SIZE} MAX_EPOCHS=${MAX_EPOCHS} BATCH_SIZE=${BATCH_SIZE} EVOL_CONFIG=${EVOL_CFG} \
      sbatch --parsable --job-name=mae_evolve_seed17 ${depopt} \
